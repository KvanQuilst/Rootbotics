...
--                                                                           --
-- This file contains the implementation of the Electric Eyrie faction from  --
-- Root: The Clockwork Expansion for use in The Rootbotics Tool.             --
--                                                                           --
...
package body Root.Eyrie is
   ...
   begin
      ...
      if Meeple_Supply = MEEPLE_MAX and then Roost_Supply = ROOST_MAX then
         New_Line;
      ...
      for I in Decrees'Range loop
         if I /= Max and then Decrees (I) = Decrees (Max) then
            Tie := True;
      ...
      Put_Line ("--  Recruit");
      for I in Decrees'Range loop
         if Decrees (I) > 0 and then Meeple_Supply > 0 then
            Recruit (I, M);
         end if;
      end loop;
      ...
      for I in Decrees'Range loop
         if Decrees (I) > 0 then
            Battle (I, M, Max = I and then not Tie);
         end if;
      end loop;
   ...
   begin
      ...
      for J in Priority'Range loop
         ...
         if M.Clearings (J).C_Suit = S and then Roosts (J) then
            Clearings (C_Idx) := J;
            C_Idx := C_Idx + 1;

         -- Bird Decrees --
         elsif S = Bird and then Roosts (J) then
            Clearings (C_Idx) := J;
            C_Idx := C_Idx + 1;
         end if;
      end loop;
      ...
      if C_Idx > 2 then
         ...
         if Character'Pos (Option) - 96 = C_Idx then
            ...
            begin
               ...
               for J in Roosts'Range loop
                  if M.Clearings (J).C_Suit = S and then
                     Roosts (J) and then
                     Meeples (J) <= Min
                  then
                     Min := Meeples (J);
         ...
         -- Multiple clearings with most enemies --
            ...
            -- Choose multiple clearings, pick lowest warriors/priority --
            declare
               PL : Priority_List;
               Min : Integer := MEEPLE_MAX;
               Min_P : Integer := 1;
            begin
               ...
               for I in PL'Range loop
                  if PL (I) /= 0 then
                     if M.Clearings (PL (I)).C_Suit = S and then
                        Roosts (PL (I)) and then
                        Meeples (PL (I)) <= Min
                     then
                        Min := Meeples (PL (I));
                        Min_P := PL (I);
   ...
   begin
      ...
      declare
         Opts : constant Char_Arr := Get_Options (Options (1 .. Count));
      begin
         ...
         for I in Opts'Range loop
            ...
            if Meeples (Val) > Max and then Val > Max_Idx then
               Max := Meeples (Val);
               Max_Idx := Val;
            end if;
         end loop;
      ...
      if Max > 0 then
         for I in M.Clearings (Max_Idx).Neighbors'Range loop
            if M.Clearings (Max_Idx).Neighbors (I) /= 0 then
               Put_Line
                  ("What is the total number of enemy PIECES in clearing" &
               ...
               if not Roosts (M.Clearings (Max_Idx).Neighbors (I)) and then
                  Val <= Min
               then
                  Min_R := Val;
         ...
         Put_Line ("How many warriors do the " & Name & " need to rule " &
                   "clearing" & Max_Idx'Image & "? ");
         Val := Get_Integer (0, 30);
         -- TODO Check if eyrie remains in rule if moved --
         Val := (if Val > Decrees (S) then Val else Decrees (S));
         Val := Meeples (Max_Idx) - Val;
         Put_Line ("Move" & Val'Image & " warriors from clearing" &
                   Max_Idx'Image & " to clearing" & Min_Idx'Image);
      end if;
   end Move;
